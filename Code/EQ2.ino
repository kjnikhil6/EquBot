...
int acc_calibration_value = 8874;                            //Enter the accelerometer calibration value
...
//  }
  ...
  angle_gyro -= gyro_yaw_data_raw * 0.000000003;                            //Compensate the gyro offset when the robot is rotating

  angle_gyro = angle_gyro * 0.9996 + angle_acc * 0.0004;                    //Correct the drift of the gyro angle with the accelerometer angle
  //angle_gyro = angle_gyro * 0.9998 + angle_acc * 0.0002;
//Serial.print("theta:");
...
//  pid_last_d_error = pid_error_temp;                                        //Store the error for the next loop
  ...
  thetaB=angle_gyro;//(thetaB_degree)* (pi / 180);
...
//  }
  ...
  if(pid_output_left > 0)pid_output_left = 405 - (1/(pid_output_left + 9)) * 3500;
...
//  else if(pid_output_right < 0)pid_output_right = -405 - (1/(pid_output_right - 9)) * 5500;
  ...
  if(pid_output_left > 0)left_motor = 460 - pid_output_left;
  else if(pid_output_left < 0)left_motor = -460 - pid_output_left;
  else left_motor = 0;
//
